...
- [ ] 在現有 `index.html` 或 `App.vue` 加入導航連結
- [ ] 新增「即時翻譯」入口，與「會議紀錄」並列
- [ ] 確保導航樣式與現有系統一致
...
- [ ] 撰寫 REST API 整合測試（pytest + httpx）
- [ ] 撰寫 WebSocket 端點測試（pytest + websockets）
- [ ] 測試完整錄音流程（啟動 → 串流 → 停止 → 查詢）

**驗收標準：** 後端測試覆蓋率 ≥ 80%

### 8.3 端對端測試

- [ ] 撰寫 Playwright E2E 測試
- [ ] 測試即時錄音完整流程
- [ ] 測試列表篩選與分頁
- [ ] 測試詳情頁與匯出功能
